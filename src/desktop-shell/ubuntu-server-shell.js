...
const SHELL_SESSION_ID_STORAGE_KEY = "ubuntu-server.shell.session-id.v1";
const SHELL_TIPS_SERVICE_STORAGE_KEY = "ubuntu-server.shell.tips-service.v1";
const SHELL_HISTORY_MAX_ENTRIES = 300;
const LLM_SHELL_ENDPOINT = "/worker/";
const LLM_MAX_MUTATIONS_PER_RESPONSE = 8;
const LLM_MAX_MUTATION_PATH_CHARS = 220;
const LLM_MAX_MUTATION_CONTENT_CHARS = 1200;
const LLM_MAX_MUTATION_TOTAL_CONTENT_CHARS = 4000;
const LLM_ALLOWED_MUTATION_ACTIONS = new Set([
  "mkdir",
  "create-dir",
  "create-directory",
  "create",
...
const LLM_ALLOWED_UI_EVENT_TYPES = new Set(["OPEN_APP"]);
const SHELL_SESSION_ID_PATTERN = /^[a-z0-9][a-z0-9._:-]{7,95}$/i;
...
function hasSensitiveCommandMaterial(commandText) {
  const normalized = typeof commandText === "string" ? commandText : String(commandText || "");
  ...
  return (
    /\b(password|passwd|pwd)\b\s*[:=]/i.test(normalized) ||
    /\b(token|secret|api[_-]?key|authorization)\b\s*[:=]/i.test(normalized) ||
...
function generateShellSessionId() {
  if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") {
    return crypto.randomUUID();
  }

  const timeToken = Date.now().toString(36);
  const randomToken = Math.random().toString(36).slice(2, 14);
  return `sess-${timeToken}-${randomToken}`;
}

function readShellSessionIdFromStorage(storageRef) {
  if (!storageRef || typeof storageRef.getItem !== "function") {
    return "";
  }

  try {
    const rawValue = storageRef.getItem(SHELL_SESSION_ID_STORAGE_KEY);
    const candidate = typeof rawValue === "string" ? rawValue.trim() : "";
    return SHELL_SESSION_ID_PATTERN.test(candidate) ? candidate : "";
  } catch {
    return "";
  }
}

function writeShellSessionIdToStorage(storageRef, sessionId) {
  if (!storageRef || typeof storageRef.setItem !== "function") {
    return;
  }

  try {
    storageRef.setItem(SHELL_SESSION_ID_STORAGE_KEY, sessionId);
  } catch {
    // Ignore storage write failures in restricted/private sessions.
  }
}

function resolveShellSessionId(storageRef) {
  const persisted = readShellSessionIdFromStorage(storageRef);

  if (persisted) {
    return persisted;
  }

  const generated = generateShellSessionId();
  writeShellSessionIdToStorage(storageRef, generated);
  return generated;
}

function normalizeTipsServiceState(source) {
...
function readTipsServiceStateFromStorage(storageRef) {
  if (!storageRef || typeof storageRef.getItem !== "function") {
    return normalizeTipsServiceState(null);
  }

  try {
    const rawValue = storageRef.getItem(SHELL_TIPS_SERVICE_STORAGE_KEY);
    ...
    const parsed = JSON.parse(rawValue);
    return normalizeTipsServiceState(parsed);
  } catch {
    return normalizeTipsServiceState(null);
  }
}

function writeTipsServiceStateToStorage(storageRef, nextState) {
  if (!storageRef || typeof storageRef.setItem !== "function") {
    return;
  }
  ...
  try {
    storageRef.setItem(
      SHELL_TIPS_SERVICE_STORAGE_KEY,
      JSON.stringify(normalizeTipsServiceState(nextState)),
    );
  } catch {
    // Ignore storage write failures in restricted/private sessions.
  }
...
export function createUbuntuServerShell({
  ...
  const llmSessionId = resolveShellSessionId(storageRef);
  ...
  let tipsServiceState = readTipsServiceStateFromStorage(storageRef);
  ...
  function getOutputLineCount() {
    ...
    return activeOutput.querySelectorAll(".ubuntu-shell__line").length;
  }

  function getLastOutputLineAfter(startIndex = 0) {
    if (!(activeOutput instanceof HTMLElement)) {
      return null;
    }

    const lines = activeOutput.querySelectorAll(".ubuntu-shell__line");
  ...
  function getOutputLineAt(index) {
    if (!(activeOutput instanceof HTMLElement)) {
      return null;
    }
  ...
  function isTipsServiceRunning() {
    return tipsServiceState.running === true;
  }

  function updateTipsServiceState(nextState) {
    tipsServiceState = normalizeTipsServiceState(nextState);
    writeTipsServiceStateToStorage(storageRef, tipsServiceState);
  }

  function normalizeInlineTipText(value) {
  ...
  function appendInlineTipToLastOutputLine(tipText, startIndex = 0) {
    if (!isTipsServiceRunning()) {
      return;
    }

    const normalizedTip = normalizeInlineTipText(tipText);

    if (!normalizedTip) {
      return;
    }

    const targetLine = getLastOutputLineAfter(startIndex) || getOutputLineAt(startIndex - 1);

    if (!(targetLine instanceof HTMLElement)) {
      return;
    }

    const existingTipNode = targetLine.querySelector(".ubuntu-shell__inline-tip");
    existingTipNode?.remove();

    const tipNode = document.createElement("span");
    tipNode.className = "ubuntu-shell__inline-tip";
    tipNode.textContent = `  // ${normalizedTip}`;
    targetLine.append(tipNode);
  }
  ...
  function pushCommandToHistory(commandText) {
    ...
    if (hasSensitiveCommandMaterial(normalized)) {
  ...
  function sanitizeLlmSuggestions(rawSuggestions) {
    const source = Array.isArray(rawSuggestions) ? rawSuggestions : [];
    const suggestions = [];

    for (const rawSuggestion of source.slice(0, 5)) {
      if (!rawSuggestion) {
        continue;
      }
      ...
      if (typeof rawSuggestion === "string") {
        commandText = rawSuggestion.trim();
      } else if (typeof rawSuggestion === "object" && !Array.isArray(rawSuggestion)) {
        commandText = String(
      ...
      const normalizedCommand =
        commandText.length > 40 ? `${commandText.slice(0, 37)}...` : commandText;
      const normalizedDescription =
        descriptionText.length > 140 ? `${descriptionText.slice(0, 137)}...` : descriptionText;
  ...
  async function parseLlmResponse(response) {
    ...
    const normalizedResponse = {
      ...
      tip:
        typeof parsedBody.tip === "string"
          ? parsedBody.tip
          : typeof parsedBody.explanation === "string"
            ? parsedBody.explanation
            : "",
      ...
      suggestions: sanitizeLlmSuggestions(
        parsedBody.suggestions || parsedBody.command_suggestions || parsedBody.hints || [],
      ),
    };
    const hasNoVisibleOutput =
      !normalizedResponse.stdout.trim() &&
      !normalizedResponse.stderr.trim() &&
      !normalizedResponse.tip.trim() &&
  ...
  function printHelp() {
    ...
    appendOutputLine("service tips <op>    Manage inline tips (start|stop|enable|disable|status)");
    appendOutputLine("");
    appendOutputLine("Unknown commands automatically route to /worker/ on this domain.", "status");
    appendOutputLine("");
  ...
  function isSafeMutationPathCandidate(pathCandidate) {
    ...
    return (
      trimmed.length > 0 &&
      trimmed.length <= LLM_MAX_MUTATION_PATH_CHARS &&
      !trimmed.includes("\n") &&
      !trimmed.includes("\r")
    );
  }

  function sanitizeLlmMutations(rawMutations = []) {
    ...
    for (const rawMutation of source.slice(0, LLM_MAX_MUTATIONS_PER_RESPONSE)) {
      ...
      if (!action || !LLM_ALLOWED_MUTATION_ACTIONS.has(action)) {
        dropped += 1;
        continue;
      }
      ...
      if (!isSafeMutationPathCandidate(targetPathCandidate)) {
        dropped += 1;
        continue;
      }

      const normalizedTargetPath = resolveTargetPath(targetPathCandidate);
      ...
      if (action === "move" || action === "mv" || action === "rename") {
        const destinationPathCandidate =
          rawMutation.to ||
          rawMutation.destination ||
          rawMutation.newPath ||
          rawMutation.nextPath;

        if (!isSafeMutationPathCandidate(destinationPathCandidate)) {
          dropped += 1;
          continue;
        }

        const normalizedDestinationPath = resolveTargetPath(destinationPathCandidate);

        if (!normalizedDestinationPath || normalizedDestinationPath === "/") {
          dropped += 1;
          continue;
        }

        sanitizedMutation.to = normalizedDestinationPath;
      }
      ...
      if (
        ...
        if (content.length > LLM_MAX_MUTATION_CONTENT_CHARS) {
          content = content.slice(0, LLM_MAX_MUTATION_CONTENT_CHARS);
          truncated += 1;
        }

        if (totalContentChars + content.length > LLM_MAX_MUTATION_TOTAL_CONTENT_CHARS) {
          dropped += 1;
          continue;
        }
    ...
    if (source.length > LLM_MAX_MUTATIONS_PER_RESPONSE) {
      dropped += source.length - LLM_MAX_MUTATIONS_PER_RESPONSE;
    }
  ...
  async function executeLlmUiEvents(uiEvents = []) {
    for (const uiEvent of uiEvents) {
      ...
      if (!LLM_ALLOWED_UI_EVENT_TYPES.has(eventType)) {
        continue;
      }
  ...
  async function processViaLLM(rawCommand, { command = "", args = [] } = {}) {
    const stopIndicator = createLoadingIndicator();

    try {
      const response = await postToLlmShell({
        command: rawCommand,
        command_token: command || splitCommand(rawCommand)[0] || "",
        argv: Array.isArray(args) ? args : [],
        args_text: Array.isArray(args) ? args.join(" ") : "",
      ...
      if (Array.isArray(llmResponse.suggestions) && llmResponse.suggestions.length > 0) {
        appendOutputLine("Try one of these:", "status");

        for (const suggestion of llmResponse.suggestions) {
          const commandLabel = String(suggestion.command || "").trim();

          if (!commandLabel) {
            continue;
          }

          const descriptionLabel = String(suggestion.description || "").trim();
          appendOutputLine(
            descriptionLabel
              ? `  ${commandLabel}  - ${descriptionLabel}`
              : `  ${commandLabel}`,
            "status",
          );
        }
      ...
      if (Array.isArray(llmResponse.vfs_mutations)) {
        const mutationGuard = sanitizeLlmMutations(llmResponse.vfs_mutations);

        if (mutationGuard.dropped > 0) {
          appendOutputLine(
            `vfs: dropped ${mutationGuard.dropped} invalid or excess mutation(s) from LLM response`,
            "error",
          );
        }

        if (mutationGuard.truncated > 0) {
          appendOutputLine(
            `vfs: truncated ${mutationGuard.truncated} oversized mutation content field(s)`,
            "status",
          );
        }

        for (const mutation of mutationGuard.mutations) {
  ...
  function buildLocalCommandTip(command, wasSuccessful) {
    if (command === "help") {
      return "Displayed available built-in commands.";
    }

    if (command === "history") {
      return wasSuccessful
        ? "Printed your persisted command history."
        : "No command history entries were available.";
    }

    if (command === "clear") {
      return "Cleared terminal output for this session.";
    }

    if (command === "date") {
      return "Printed current local date and time.";
    }

    if (command === "uptime") {
      return "Showed how long this shell instance has been running.";
    }

    if (command === "whoami") {
      return "Printed the active shell user.";
    }

    if (command === "hostname") {
      return "Printed the current host name.";
    }

    if (command === "uname") {
      return "Printed kernel identity information.";
    }

    if (command === "pwd") {
      return "Printed the current working directory.";
    }

    if (command === "cd") {
      return wasSuccessful
        ? "Changed the current working directory."
        : "Directory change failed because the target path was unavailable.";
    }

    if (command === "ls") {
      return wasSuccessful
        ? "Listed visible files and directories."
        : "Directory listing failed for the requested path.";
    }

    if (command === "cat") {
      return wasSuccessful
        ? "Displayed file contents in the terminal."
        : "File read failed for the requested path.";
    }

    if (command === "open") {
      return wasSuccessful
        ? "Forwarded the target to the GUI app layer."
        : "Open request failed for the selected target.";
    }

    if (command === "oslist") {
      return "Displayed detected operating systems for this machine.";
    }

    if (command === "boot" || command === "reboot") {
      return wasSuccessful
        ? "Queued a system reboot request."
        : "System reboot request was rejected due to invalid target.";
    }

    if (command === "shutdown" || command === "poweroff" || command === "halt") {
      return "Stopped shell services and moved to halted state.";
    }

    if (command === "msconfig") {
      return "This command is GUI-only in the current shell profile.";
    }

    if (command === "service") {
      return wasSuccessful
        ? "Updated service state for this shell session."
        : "Service command failed due to invalid arguments.";
    }

    return wasSuccessful
      ? `Executed local command '${command}'.`
      : `Command '${command}' returned an error in local fast path.`;
  }
  ...
  async function executeLocalCommand(command, args) {
    ...
    if (command === "service") {
      const serviceName = normalizeToken(args[0]);
      const operation = normalizeToken(args[1] || "status");

      if (serviceName !== "tips") {
        appendOutputLine("service: only `tips` is available in this shell.", "error");
        return false;
      }

      if (operation === "status") {
        const runningLabel = tipsServiceState.running ? "running" : "stopped";
        const enabledLabel = tipsServiceState.enabled ? "enabled" : "disabled";
        appendOutputLine(`tips.service is ${runningLabel} (${enabledLabel})`);
        return true;
      }

      if (operation === "start") {
        updateTipsServiceState({
          ...tipsServiceState,
          running: true,
        });
        appendOutputLine("tips.service started");
        return true;
      }
      ...
      if (operation === "stop") {
        ...
        appendOutputLine("tips.service stopped");
        return true;
      }
      ...
      if (operation === "enable") {
        ...
        appendOutputLine("tips.service enabled and started");
        return true;
      }
      ...
      if (operation === "disable") {
        ...
        appendOutputLine("tips.service disabled and stopped");
        return true;
      }

      appendOutputLine(
        "Usage: service tips <start|stop|enable|disable|status>",
        "error",
      );
      return false;
    }
  ...
  async function executeCommand(commandText) {
    ...
    const commandToken = String(parts[0] || "").trim();
    const command = normalizeToken(commandToken);
    ...
    if (LOCAL_COMMANDS.has(command)) {
      ...
      return {
        tip: buildLocalCommandTip(command, localCommandSucceeded),
      };
    }

    return processViaLLM(commandText, {
  ...
  function renderTerminal() {
    ...
    const submitHandler = (event) => {
      ...
      activeInput.value = "";
      focusInput();
      const queuedCommand = nextCommand;
      commandQueue = commandQueue
        .catch(() => {})
        .then(async () => {
          const outputStartIndex = getOutputLineCount();
          ...
          try {
            const commandResult = await executeCommand(queuedCommand);
            appendInlineTipToLastOutputLine(commandResult?.tip || "", outputStartIndex);
          } catch {
